...
const TIPO_KEY: Record<string, string> = {
  PC1: 'pc', PC2: 'pc', PC3: 'pc', PC4: 'pc',
  EP: 'ep', EF: 'ef', ES: 'es', Labo: 'labo', Mono: 'mono',
}
...
function TipoCategoryCard({
  ...
  const key = TIPO_KEY[tipo] ?? 'default'
  const preview = expanded ? materials : materials.slice(0, 3)

  return (
    <div className={`tipo-category-card tipo-category-card-${key}`}>
      {/* Header */}
      <div className={`px-4 pt-4 pb-3 flex items-center justify-between tipo-category-header-${key}`}>
        ...
        <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0 bg-white/70">
          {TIPO_ICON[tipo] ?? '📄'}
        </div>
      </div>
        ...
        {preview.map(m => {
          ...
          return (
            <li key={m.id} className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition-colors group">
              <div className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 card-author-avatar">
                {initials}
              </div>
              ...
              <span className="text-[10px] font-mono font-semibold px-1.5 py-0.5 rounded-md flex-shrink-0 card-ciclo-pill">
                {m.ciclo}
              </span>
              <button
                onClick={() => onDownload(m)}
                className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all cursor-pointer opacity-70 hover:opacity-100 tipo-category-header-${key}`}
                title="Descargar"
              >
      ...
      {materials.length > 3 && (
        <button
          ...
          className={`flex items-center justify-center gap-1.5 py-2.5 text-xs font-bold transition-colors cursor-pointer border-t tipo-category-header-${key}`}
        >
          {expanded ? (
...
export function CourseDetailView({
  ...
  return (
    <div className="animate-fade-in min-h-screen bg-slate-50">
      {/* Course Sub-Hero */}
      <section className="course-detail-subhero">
          ...
          <div className="flex flex-wrap gap-2 mt-5">
            {[...grouped.keys()].map(tipo => {
              ...
              return (
                ...
                >
                  <span className={`w-2 h-2 rounded-full tipo-badge-dot-${key}`} />
                  {tipo}
